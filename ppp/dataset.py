...
class PPPDataset(Dataset):
    ...
    def __init__(
        ...
        if use_mask_captioned_data:
            ...
            self.instance_images_path = sorted(
                list(set(possibily_src_images)),
                key=lambda x: int(str(Path(x).stem).split(".")[0]),
            )
            self.captions = [
        ...
        print("Captions:")
        print(self.captions, "these are irrelavent if you are using templates")
    ...
    def __getitem__(self, index):
        ...
        if self.use_template:
            ...
            text = random.choice(self.templates).format(self.main_token)
        else:
        ...
        if self.is_extended:
            texts = pp_extend(
                ...
                extend_amount=self.extend_amount,
            )
        else:
            texts = [text]
